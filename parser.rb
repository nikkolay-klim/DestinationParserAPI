...
class DestinationParser
    ...
    def distance_between_cities(origin, destination)
        ...
        begin
            raise TypeError, 'Origin must be filled' unless origin.is_a? String
            raise TypeError, 'Destination to must be filled' unless destination.is_a? String
            ...
            raise NoMethodError, 'Please provide correct origin' if parsed_data['origin_addresses'][0].empty?
            raise NoMethodError, 'Please provide correct destination' if parsed_data['destination_addresses'][0].empty?
